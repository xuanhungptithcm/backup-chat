...
io.on('connection', function (socket) {
  // Định Danh Chính Mình
  socket.on('getconnect', (req) => {
    usersSocket[req.id] = socket.id;
    socket.emit('provider', 'Bạn Đã Được Định Danh');
  })
  // Tìm User !
...
// Kiểm Tra Có Mình Có Kết Bạn Với Nó Chưa
  ...
  // Nhắn Tin Ở Đây Nhé Mọi Người
  socket.on('message', (req) => {
    io.to(usersSocket[req.idFriend]).emit('messageResponse',req.message);
  })

  socket.on('disconnect', function () {
    console.log('user has socket ID ', socket.id ,' disconnected');
  });
});
